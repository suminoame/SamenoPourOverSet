...
- 2026/08/15: Manual v1.1.0 コーヒーカップにミルクを入れた際の機能を追加
- 2026/07/28: Manual v1.0.1 豆情報およびレシピデータの登録方法を追記
...
またコーヒーを入れた状態で、さめのワークス「Siphon Coffee Set」や「VRC Tea Time」等のミルクをいれるとラテを作ることができます。

上記の他、「Milk」という名称を含むCollider付きワールドパーティクルや、特定のContactを設定したアバターのミルクパーティクルに反応します。Contactを用いたアバターパーティクルの設定は同梱のマニュアル「アバターパーティクル（ミルク）設定方法」をご確認ください。

※アバターパーティクルはご自身で設定できる方向けのオプション機能となります。セットアップは本アセットのサポート対象外となりますのでご了承ください。
...
日付を指定しない場合は、ワールドインスタンスが建てられた日から「エイジング日数（Aging Days）」で設定された日数だけ前の日付が焙煎日欄に自動入力されます。
